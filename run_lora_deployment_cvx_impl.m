function results = run_lora_deployment_cvx_impl()
    ...
    params = audit_and_adjust_parameters(params);
    state = build_scenario(params);
    results = solve_all_intervals(params, state);
    results.params = params;

    validate_final_results(results, params);
    print_summary(results, params);
    intervalll = results.intervals{1};
    figure
    plot(1:numel(intervalll.history), intervalll.history, '-o', 'LineWidth', 1.8, 'MarkerSize', 6);
    grid on;
    xlabel('Iteration');
...
function params = default_parameters()
    ...
    params.pUGVcomp = 165.0;
    params.pUAVcomp = 85.0;
    params.PmaxCCS = 28.0;
    params.PmaxUGV = 9.0;

    params.dData = 150.0e6 * ones(params.N, 1);
    params.dCCS = 10.0 * 8.0e6 * ones(params.N, 1);
    params.dUGV = params.dCCS;

    params.Fup = params.dData .* params.fup;
    params.FferUGV = [2.70e17; 3.60e17];
    params.FferGU = zeros(params.G, params.U, params.N);
    params.FferGU(:, :, 1) = [1.55e17, 1.70e17, 2.00e17, 2.20e17; 2.10e17, 2.35e17, 1.80e17, 2.45e17];

    params.g0CCS = 2.0e-4;
    params.g0UGV = 6.0e-5;
    params.pathLossExpCCS = 2.35;
    params.pathLossExpUGV = 2.45;
    params.kappaCCS = 5.0;
    params.kappaUGV = 3.0;
    params.lambda = 0.125;
    params.antSpacing = params.lambda / 2.0;
    params.sigmaCCS2 = 2.0e-11;
    params.sigmaUAV2 = 8.0e-11;

    params.ccsPos = [0.0; 0.0; 80.0];
    params.uavHeight = 60.0;

    params.outerMaxIter = 18;
    params.outerMinIter = 14;
    params.outerTol = 5.0e-5;
    params.updateDamping = 0.3;
...
function params = audit_and_adjust_parameters(params)
    params.adjustments = {};
    params.adjustments{end + 1} = 'Used d_data = 150e6 as requested and ignored the extra 15e6 workload note for F_fer derivation.';
    params.adjustments{end + 1} = 'Set d_CCS = d_UGV = 10 MB to make communication energy visible while keeping the problem feasible.';
    params.adjustments{end + 1} = 'Kept sub-second fixed overheads because the original 179 s and 336.1 s values violate the interval-delay constraint.';
    params.adjustments{end + 1} = 'Introduced asymmetric UGV and UAV workloads so optimal GPU allocations are no longer identical across all nodes.';
    params.adjustments{end + 1} = 'Weakened the wireless links and tightened tau so communication power is no longer unrealistically close to zero.';
end
...
function state = build_scenario(params)
    ...
    uavBase = [-78.0, -24.0, 34.0, 82.0;
                58.0,  16.0, 24.0, 68.0];
    uavVel = [0.4, -0.2, 0.3, -0.3;
              0.1,  0.2, 0.1,  0.2];
...
function sol = solve_single_interval(params, state, n)
    ...
    gamma = [0.55; 0.45];
    p = 0.75 * params.PmaxUGV * ones(params.G, 1);
    ...
    for iter = 1:params.outerMaxIter
        ...
        pCand = solve_s6_power(params, state.hUGV, n, metrics, beta, gamma, alphaU);
...
function Wbest = solve_s2_beamforming(params, hCCS, n, metrics)
    ...
    if any(etaMax <= params.epsVal)
        error('S2 infeasible: no admissible CCS transmission budget.');
    end

    etaHigh = min(etaMax);
    etaLow = max(params.epsVal, 0.25 * etaHigh);
    for iter = 1:30
    ...
    etaStar = fminbnd(objFun, etaHigh, min(etaMax), opts);
...
function beta = solve_s4_association(params, hUGV, n, metrics, gamma, p, alphaU)
    ...
    rates = zeros(params.G, params.U);
    jointLatency = zeros(params.G, params.U);
    for g = 1:params.G
        for u = 1:params.U
            coeff = channel_gain_ratio(params, abs(hUGV(g, u))^2);
            rates(g, u) = gamma(g) * params.B * log2(1.0 + coeff * p(g));
            txLatency = params.dUGV(n) / max(rates(g, u), params.epsVal);
            jointLatency(g, u) = txLatency + uav_update_latency(params, n, g, u, alphaU(u));
        end
    end
    ...
    for row = 1:size(assignments, 1)
        ...
        for g = 1:params.G
            ...
            if isempty(users)
                ...
                minRate = min(minRate, min(rates(g, users)));
            end
        ...
        if feasible && minRate > bestMetric + 1.0e-9
            bestMetric = minRate;
            beta = candidate;
        end
    end
...
function alphaU = solve_s7_alpha_uav(params, n, metrics, beta)
    ...
    for u = 1:params.U
        ...
        alphaU(u) = min(params.Ku, max(1, ceil(params.muUAV * params.FferGU(g, u, n) / (params.fUAV * rhs))));
    end
end
...
function ok = is_feasible_metrics(params, metrics, W, beta, gamma, p, alphaCCS, alphaG, alphaU)
    ...
    ok = ok && all(alphaU >= 1 & alphaU <= params.Ku);
...
function validate_final_results(results, params)
    history = results.intervals{1}.history(:);
    diffs = diff(history);
    if any(diffs > 1.0e-6)
        error('Objective history is not monotonically non-increasing.');
    end
    if numel(history) < 8
        error('Convergence is too fast; the AO curve does not provide a meaningful iterative trend.');
    end
    if history(1) - history(end) <= 5.0
        error('AO curve variation is too small.');
    end

    metrics = results.intervals{1}.metrics;
    if any(metrics.ccsTx >= params.tth)
        error('CCS-to-UGV transmission latency must stay below 10 s.');
    end
    if max(metrics.txPowerCCS, max(results.intervals{1}.p)) <= 0.2
        error('Communication power is unrealistically small; scenario needs tighter communication coupling.');
    end
end
...
function print_summary(results, params)
    ...
    % fprintf('Parameter adjustments:\n');
    % for idx = 1:numel(params.adjustments)
    %     fprintf('  - %s\n', params.adjustments{idx});
    % end
end
