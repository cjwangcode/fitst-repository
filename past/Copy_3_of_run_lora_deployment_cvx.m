function results = run_lora_deployment_cvx()
    clc;
    close all;
    clear
    rng(1);
...
function params = default_parameters()
    ...
    params.dData = 100e6 * ones(params.N, 1);
    params.dCCS = 8.0e6 * ones(params.N, 1);
    params.dUGV = 1.2e7 * ones(params.N, 1);

    params.Fup = 2.07e10 * ones(params.N, 1);
    params.FferCCS = 7.0e10 * ones(params.N, 1);
    params.FferUGV = repmat([3.2e10; 2.8e10], 1, params.N);

    params.FferGU = zeros(params.G, params.U, params.N);
    params.FferGU(:, :, 1) = [9.0e9, 8.2e9, 7.8e9, 7.4e9;
                              8.5e9, 8.8e9, 7.6e9, 8.1e9];

    params.g0 = 4.0;
    params.pathLossExpCCS = 2.0;
    params.pathLossExpUGV = 2.3;
    params.kappaCCS = 10.0;
    params.kappaUGV = 3.0;
    params.lambda = 0.125;
    params.antSpacing = params.lambda / 2.0;
    params.sigmaCCS2 = 1e-12;
    params.sigmaUAV2 = 2e-7;
...
function sol = solve_single_interval(params, state, n)
    ...
    gamma = [0.82; 0.18];
    p = 0.95 * params.PmaxUGV * ones(params.G, 1);
