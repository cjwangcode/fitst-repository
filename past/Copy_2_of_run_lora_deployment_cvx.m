...
function params = default_parameters()
    ...
    params.pCCScomp = 320.0;
    params.pUGVcomp = 220.0;
    params.pUAVcomp = 120.0;

    params.PmaxCCS = 20.0;
    params.PmaxUGV = 8.0;
    params.betaMax = 3;

    params.dData = 150e6 * ones(params.N, 1);
    params.dCCS = 0.875 * 8e6 * ones(params.N, 1);
    params.dUGV = 15.0 * 8e6 * ones(params.N, 1);

    params.Fup = 150e6 * 2.07e10 * ones(params.N, 1);
...
function sol = solve_single_interval(params, state, n)
    alphaCCS = 6;
    alphaG = 6 * ones(params.G, 1);
    alphaU = 4 * ones(params.U, 1);
    beta = initial_association(params);
    gamma = [0.58; 0.42];
    p = 0.65 * params.PmaxUGV * ones(params.G, 1);
...
function alphaCCS = solve_s1_alpha_ccs(params, n, metrics)
    A = params.Fup(n) * params.muCCS / params.fCCS;
    lowerBound = 1;
    for g = 1:params.G
