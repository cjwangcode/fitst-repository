...
function params = default_parameters()
    ...
    params.pCCScomp = 200.0;
    params.pUGVcomp = 100.0;
    params.pUAVcomp = 600.0;
    params.PmaxCCS = 18.0;
    params.PmaxUGV = 6.0;

    params.dData = 150.0e6 * ones(params.N, 1);
    params.dCCS = 8.0 * 8.0e6 * ones(params.N, 1);
    ...
    params.g0CCS = 1.0e-2;
    params.g0UGV = 4.0e-3;
    params.pathLossExpCCS = 2.15;
    params.pathLossExpUGV = 2.25;
    params.kappaCCS = 8.0;
    params.kappaUGV = 5.0;
    params.lambda = 0.125;
    params.antSpacing = params.lambda / 2.0;
    params.sigmaCCS2 = 3.0e-12;
    params.sigmaUAV2 = 8.0e-12;

    params.ccsPos = [0.0; 0.0; 80.0];
    params.uavHeight = 60.0;

    params.outerMaxIter = 12;
    params.outerMinIter = 20;
...
function params = audit_and_adjust_parameters(params)
    ...
    params.adjustments{end + 1} = 'Set d_CCS = d_UGV = 8 MB; the 0.875 MB payload in the parameter sheet is too small for the intended latency-energy tradeoff.';
    params.adjustments{end + 1} = 'Compressed fixed overheads from 179 s and 336.1 s to sub-second values; otherwise the paper constraints are infeasible before optimization starts.';
    params.adjustments{end + 1} = 'Raised effective CCS, UGV, and UAV compute throughputs to cluster-level values so all delays remain in the same order of magnitude as the 10 s CCS-UGV deadline.';
end
...
function state = build_scenario(params)
    ...
    uavBase = [-32.0, -14.0, 16.0, 34.0;
                24.0,  10.0, 18.0, 30.0];
    uavVel = [0.5, -0.3, 0.4, -0.4;
              0.1,  0.2, 0.1,  0.2];
...
function sol = solve_single_interval(params, state, n)
    alphaCCS = 2;
    alphaG = 2 * ones(params.G, 1);
    alphaU = 2 * ones(params.U, 1);
    beta = initial_association(params);
    gamma = [0.60; 0.40];
    p = 0.85 * params.PmaxUGV * ones(params.G, 1);
...
function Wbest = solve_s2_beamforming(params, hCCS, n, metrics)
    ...
    etaHigh = min(etaMax);
    etaLow = max(params.epsVal, 0.1 * etaHigh);
    for iter = 1:30
